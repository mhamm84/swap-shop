...
(define-non-fungible-token swapshop-test-nft uint)
...
(define-read-only (get-owner (token-id uint))
    (ok (nft-get-owner? swapshop-test-nft token-id))
)
...
(define-public (transfer (token-id  uint) (sender principal) (recipient principal))
    (begin 
        ...
        ;; #[filter(recipient)]
        (nft-transfer? swapshop-test-nft token-id sender recipient)
    )
)
...
(define-public (mint (recipient principal))
    (let
        ...
        ;; #[filter(recipient)]
        (try! (nft-mint? swapshop-test-nft next-token-id recipient))
